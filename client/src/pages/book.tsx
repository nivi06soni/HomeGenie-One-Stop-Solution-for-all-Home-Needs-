  ...
  const { data: specificProvider, isLoading: specificProviderLoading } = useQuery<ServiceProvider>({
    ...
    onSuccess: (provider) => {
      if (provider && !selectedCategoryId) {
        setSelectedCategoryId(provider.categoryId);
      }
    },
  });
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div 
                    className={`border-2 rounded-xl overflow-hidden ${
                      selectedProviderId === specificProvider.id ? 'border-primary' : 'border-transparent'
                    }`}
                    onClick={() => handleProviderSelect(specificProvider.id)}
                  >
                    <ProviderCard provider={specificProvider} />
                  </div>
                </div>
          ...
          <div className="flex justify-end">
            <Button 
              ...
              disabled={!selectedCategoryId || !selectedProviderId || !selectedTime || !description || isSubmitting}
            >
              {isSubmitting ? (
                <>
